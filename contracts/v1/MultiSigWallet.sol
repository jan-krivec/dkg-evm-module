...
contract MultiSigWallet {
    ...
    ownerExists(owner)
    {
        ...
        for (uint i = 0; i < owners.length; i++) {
            if (owners[i] == owner) {
                ...
                owners.pop(); // Use pop to remove the last element
                break;
            }
        }
    ...
    /// @dev Returns the confirmation status of a transaction for a specific owner.
    /// @param transactionId Transaction ID.
    /// @param owner Address of the owner.
    /// @return status Confirmation status.
    function isConfirmedBy(uint transactionId, address owner)
    public
    view
    returns (bool status)
    {
    ...
    /// @dev Returns the owners' addresses.
    /// @return ownersList List of owners' addresses.
    function getOwners() public view returns (address[] memory ownersList) {
        return owners;
    }
}
